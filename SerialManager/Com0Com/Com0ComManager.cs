...
namespace Com0Com
{
    public class Com0ComManager
    {
        ...
        public CrossoverPortPair CreatePortPair(string comPortNameA, string comPortNameB)
        {
            ...
            var pair = ParsePortPairsFromStdOut(stdOutLines.Select(l => l.Trim())).First();

            if (pair != null)
            {
                stdOutLines = _cmdRunner.RunCommandGetStdOut(
                    Path.GetDirectoryName(_com0ComSetupC),
                    _com0ComSetupC,
                    $"change CNCA{pair.PairNumber} RealPortName={comPortNameA}");

                stdOutLines = _cmdRunner.RunCommandGetStdOut(
                    Path.GetDirectoryName(_com0ComSetupC),
                    _com0ComSetupC,
                    $"change CNCB{pair.PairNumber} RealPortName={comPortNameB}");

                return GetCrossoverPortPairs().Where(p => p.PairNumber == pair.PairNumber).FirstOrDefault();
        ...
        private IEnumerable<CrossoverPortPair> ParsePortPairsFromStdOut(IEnumerable<string> lines)
        {
            ...
            var lineRegex = new Regex(@"^CNC([AB])([0-9]+) PortName=COM([0-9#]+)(,RealPortName=(COM[0-9]+))?");
            ...
            foreach (var line in lines)
            {
                ...
                var portName = string.IsNullOrWhiteSpace(match.Groups[5].Value) ? match.Groups[3].Value : match.Groups[5].Value;
    ...
    public class CrossoverPortPair
    {
        ...
        {
            return $"{PairNumber}: {PortNameA}<->{PortNameB}";
